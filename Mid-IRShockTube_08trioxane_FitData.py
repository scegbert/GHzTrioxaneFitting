...
for i, molecule in enumerate(molecules_meas):
    ...
    if molecule == 'CO':
        ...
        if H_precursor:
            data[molecule]['y_expected'] = [3E-5, 0.04, 0.08][T_which]
        else:
            data[molecule]['y_expected'] = [1E-7, 0.001, 0.075][T_which]
        ...
        if H_precursor:
            data[molecule]['y_expected'] = [0.20, 0.16, 0.10][T_which]
        else:
            data[molecule]['y_expected'] = [0.20, 0.20, 0.12][T_which]
    ...
    data[molecule]['meas_vac'] = np.fft.fftshift(np.fft.fft(np.fft.ifftshift(data[molecule]['IG_vac']))).__abs__()[
                                 :i_center + 1]  # fft and remove reflected portion
...
for ig_start in range(num_iters + 1):
    ...
    for i, molecule in enumerate(molecules_meas):
        ...
        i_fits = td.bandwidth_select_td(data[molecule]['wvn'], data[molecule]['wvn2_fit'],
                                        max_prime_factor=50)  # wavenumber indices of interest
        ...
        if data[molecule][
            # if we aren't calcuating T this time, but we did already calculate it
            'calc_T'] is False and T_fit_which is not False:
        ...
        fig, axs = plt.subplots(2, 2, sharex='col', sharey='row', figsize=(10, 4),
                                gridspec_kw={'height_ratios': [3, 1], 'width_ratios': [3, 1], 'hspace': 0.015,
                                             'wspace': 0.005})
        ...
        plt.suptitle(
            '{} at {} K, {} atm, and {}% while averaging {} IG, ~{} us post shock'.format(molecule, T_plot, P_plot,
                                                                                          y_plot, num_IGs_avg, t_plot))
        ...
        plt.savefig(os.path.abspath('') + r'/plots/{} {} averaging {}.jpg'.format(molecule, ig_start, num_IGs_avg),
                    bbox_inches='tight')
