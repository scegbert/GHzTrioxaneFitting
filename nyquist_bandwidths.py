...
fr = 1010e6-9998320
lambda_start =  3.2  # 4.3 # 3.45 # wavelength in um
lambda_stop = 4.7 # 4.6 # 3.64

nu_start = sc.c / lambda_stop * 1e6
nu_end = sc.c / lambda_start * 1e6

allowed_dnu_windows = find_allowed_nyquist_bandwidths(nu_start, nu_end)
allowed_dfr_windows_2 = find_allowed_dfr(nu_start, nu_end, fr)
...
ax.title.set_text('fr = '+str(fr/1E6)+'MHz, $\lambda$ = '+str(lambda_start)+' to '+str(lambda_stop)+' $\mu$m')
ax.plot(dfr[ind]/1000, x[ind], '.')
